...
def transcribe_audio(
    ...
    if not os.path.isfile(file_path):
        logger.error(f"File not found: {file_path}. Progress: 0%")
        DB.update_transcription_status_by_pid(
            "Error: File not found.", "", 0, pid
    )
    ...
    try:
        ...
        logger.error(f"Model loading failed: {str(model_error)}. Progress: 0%")
        DB.update_transcription_status_by_pid("Error: Model loading failed.", "", 0, pid)
        return
    ...
    try:
        ...
        logger.info("Transcribing... Progress: 50%")
        DB.update_transcription_status_by_pid("Transcribing...", "", 50, pid)

        try:
            pid_dir = OUTPUT_DIR / str(pid)
            pid_dir.mkdir(parents=True, exist_ok=True)
        except Exception as dir_error:
            logger.error(f"Directory creation failed: {str(dir_error)}. Progress: 0%")
            DB.update_transcription_status_by_pid("Error: Directory creation failed.", "", 0, pid)
            return
